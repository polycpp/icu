...
This branch builds **ICU 71** (defaults to release-71-1). To use a different
patch version, pass `-DICU_VERSION=71.1`. For other major versions, see the
...
|--------|---------|-------------|
| `ICU_VERSION` | `71.1` | ICU release version to fetch (e.g. `71.1`) |
...
| `ICU_DATA_ARCHIVE_DIR` | `${CMAKE_BINARY_DIR}/data` | Where to find `icudt71l.dat` (archive mode) |
...
./scripts/fetch-data.sh 71.1 ./data
