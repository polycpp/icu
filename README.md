...
This branch builds **ICU 77** (defaults to release-77-1). To use a different
patch version, pass `-DICU_VERSION=77.1`. For other major versions, see the
...
|--------|---------|-------------|
| `ICU_VERSION` | `77.1` | ICU release version to fetch (e.g. `77.1`) |
...
| `ICU_DATA_ARCHIVE_DIR` | `${CMAKE_BINARY_DIR}/data` | Where to find `icudt77l.dat` (archive mode) |
...
./scripts/fetch-data.sh 77.1 ./data
