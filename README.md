...
This branch builds **ICU 72** (defaults to release-72-1). To use a different
patch version, pass `-DICU_VERSION=72.1`. For other major versions, see the
...
|--------|---------|-------------|
| `ICU_VERSION` | `72.1` | ICU release version to fetch (e.g. `72.1`) |
...
| `ICU_DATA_ARCHIVE_DIR` | `${CMAKE_BINARY_DIR}/data` | Where to find `icudt72l.dat` (archive mode) |
...
./scripts/fetch-data.sh 72.1 ./data
