...
This branch builds **ICU 73** (defaults to release-73-2). To use a different
patch version, pass `-DICU_VERSION=73.1`. For other major versions, see the
...
|--------|---------|-------------|
| `ICU_VERSION` | `73.2` | ICU release version to fetch (e.g. `73.1, 73.2`) |
...
| `ICU_DATA_ARCHIVE_DIR` | `${CMAKE_BINARY_DIR}/data` | Where to find `icudt73l.dat` (archive mode) |
...
./scripts/fetch-data.sh 73.2 ./data
