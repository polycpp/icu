...
This branch builds **ICU 70** (defaults to release-70-1). To use a different
patch version, pass `-DICU_VERSION=70.1`. For other major versions, see the
...
|--------|---------|-------------|
| `ICU_VERSION` | `70.1` | ICU release version to fetch (e.g. `70.1`) |
...
| `ICU_DATA_ARCHIVE_DIR` | `${CMAKE_BINARY_DIR}/data` | Where to find `icudt70l.dat` (archive mode) |
...
./scripts/fetch-data.sh 70.1 ./data
